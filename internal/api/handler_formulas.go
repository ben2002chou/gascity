...
func buildFormulaDetail(ctx context.Context, store beads.Store, name string, paths []string, target string, vars map[string]string, validateRuntimeVars bool) (*formulaDetailResponse, error) {
	...
	compileVars, err := formulaDetailPreviewVars(ctx, store, name, paths, resolved, target, vars)
	if err != nil {
		return nil, err
	}
...
func formulaDetailPreviewVars(ctx context.Context, store beads.Store, name string, paths []string, resolved *formula.Formula, target string, vars map[string]string) (map[string]string, error) {
