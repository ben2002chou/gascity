...
func TestProcessDrainReplayUsesManifestWhenMemberWasDeletedBeforeUnitCreation(t *testing.T) {
	...
	if len(unitMembers) != 1 || unitMembers[0].ID != missingID || !convoycore.IsUnresolvedTrackedItem(unitMembers[0]) {
		t.Fatalf("unit members = %+v, want unresolved deleted member %s", unitMembers, missingID)
	}
}
