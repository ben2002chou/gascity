...
func PreparePreviewInvocation(ctx context.Context, store beads.Store, formulaName string, searchPaths []string, targetID string, userVars map[string]string) (Invocation, error) {
	...
	targetID = strings.TrimSpace(targetID)
	if store == nil {
		return Invocation{}, fmt.Errorf("graph.v2 preview requires a bead store")
	}
	target, err := store.Get(targetID)
	if err != nil {
		if errors.Is(err, beads.ErrNotFound) {
			return Invocation{}, fmt.Errorf("graph.v2 target %s not found: %w", targetID, err)
		}
		return Invocation{}, fmt.Errorf("loading graph.v2 target %s: %w", targetID, err)
	}
	if convoycore.IsTerminalStatus(target.Status) {
		return Invocation{}, fmt.Errorf("graph.v2 target %s is %s", target.ID, target.Status)
	}
	inputConvoyID := target.ID
	if target.Type != "convoy" {
		existing, err := findExistingSingleton(store, target.ID)
		if err != nil {
			return Invocation{}, fmt.Errorf("looking up singleton convoy for %s: %w", target.ID, err)
		}
		if existing.ID == "" {
			inputConvoyID = previewSingletonPrefix + target.ID
		} else {
			hasTrack, err := convoycore.HasTrack(store, existing.ID, target.ID)
			if err != nil {
				return Invocation{}, fmt.Errorf("checking singleton convoy %s track for %s: %w", existing.ID, target.ID, err)
			}
			if !hasTrack {
				return Invocation{}, fmt.Errorf("graph.v2 preview singleton convoy %s is missing track for %s", existing.ID, target.ID)
			}
			inputConvoyID = existing.ID
		}
		inv.Singleton = true
	}
	inv.Targeted = true
